...
void generateReport() {
    ...
    for (int n = 0; size_t(n) < properties.size(); n++) {
        ...
        double totalYearlyRevenue = 0.00, monthlyRevenue = 0.00, rate = temprsp.getPricePerNight();

        outfile << "Address: " << temprsp.address << endl;
        outfile << fixed << setprecision(2);

        vector <dateRange> monthWiseBookedDates;

        for (int i = 0; size_t(i) < temprsp.bookedDates.size(); i++) {
            dateRange tempDR = temprsp.bookedDates[i];
            int thisMonth = tempDR.start.month;
            if (thisMonth == temprsp.bookedDates[i].end.month) {
                monthWiseBookedDates.push_back(tempDR);
            } else {
                for(int j = thisMonth; j <= temprsp.bookedDates[i].end.month; j++) {
                    dateRange newDR;
                    if (j == thisMonth) {
                        mdDate newSD = {tempDR.start.month, tempDR.start.day};
                        mdDate newED = {tempDR.start.month, DAYS_IN_A_MONTH};
                        newDR.start = newSD, newDR.end = newED;
                    } else if (j == temprsp.bookedDates[i].end.month) {
                        mdDate newSD = {tempDR.end.month, 1};
                        mdDate newED = {tempDR.end.month, tempDR.end.day};
                        newDR.start = newSD, newDR.end = newED;
                    } else {
                        mdDate newSD = {j, 1};
                        mdDate newED = {j, DAYS_IN_A_MONTH};
                        newDR.start = newSD, newDR.end = newED;
                    }
                    monthWiseBookedDates.push_back(newDR);
                }
            }
        }

        vector<int> bookedDates;
        for (int i = 0; size_t(i) < monthWiseBookedDates.size(); i++) {
            dateRange &dr = monthWiseBookedDates[i];
            int startMonth = dr.start.month;
            int nextDateMon = monthWiseBookedDates[i+1].start.month;
            
            int startDay = (dr.end.month - dr.start.month > 0) && (startMonth > dr.start.month) ? 1 : dr.start.day;;
            int endDay = (dr.end.month - dr.start.month > 0) && (startMonth < dr.end.month) ? DAYS_IN_A_MONTH : dr.end.day;
            ...
            if (startMonth != nextDateMon) {
                outfile << MONTHS[startMonth-1] << ": " << monthlyRevenue << endl;
                
                vector <string> calenderData = getMonthCalenderData(startMonth, &bookedDates);
